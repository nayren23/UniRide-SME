...
@pytest.fixture
def get_duplicated_user(get_default_user):
    """Add another user the db"""
...
def test_get_from_db_user_not_found_with_default_user():
    with pytest.raises(UserNotFoundException):
        UserBO(
            login="bob123", firstname="Bob", lastname="Pignon", student_email="bobpignon@iut.univ-paris8.fr", password="123", gender="H", phone_number="612345678", description="Salut c'est moi Bob"
        ).get_from_db()
...
def test_validate_login_taken(get_default_user, get_duplicated_user):
    user = get_default_user
